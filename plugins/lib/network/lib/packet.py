...
class ICMP:
    ...
    def _create_packet(self, packet_id: int) -> ByteString:
        ...
        return struct.pack("bbHHh", ICMP_ECHO_REQUEST, 0, socket.htons(self._checksum(header)), packet_id, 1)


class TCP:
    def __init__(self, source: Tuple[IPv4Address, Port], destination: Tuple[IPv4Address, Port], flags: int=0, payload: Optional[ByteString]=None, size: int=512):
        self.source = source
        self.destination = destination
    ...
    def _create_packet(self) -> ByteString:
        ...
        packet = struct.pack(
            "!HHIIBBHHH", source_port, destination_port, 0, 0, 5 << 4, self.flags, self.size, 0, 0
        )
        ...
        return struct.pack(
            "!HHIIBBHHH", source_port, destination_port, 0, 0, 5 << 4, self.flags, self.size, self._checksum(packet + data), 0
        ) + self.payload


class UDP:
    def __init__(self, source: Tuple[IPv4Address, Port], destination: Tuple[IPv4Address, Port], payload: Optional[ByteString]=None):
        self.source = source
        self.destination = destination
        self.payload = payload
        self.data = self._create_packet()
    ...
    def _create_packet(self) -> ByteString:
        ...
        packet = struct.pack(
            "!HHHH", source_port, destination_port, 0, 0
        )
        ...
        return struct.pack(
            "!HHHH", source_port, destination_port, len(packet) + len(self.payload), self._checksum(packet + data)
        ) + self.payload
