...
from typing import Dict, List, AnyStr, Union, NewType, Generator
IPv4Address = NewType("IPv4Address", str)

from plugins.lib.network.lib.port import PortConnect, OpenPort


def port_connect(host: IPv4Address, port: int, timeout: int=5) -> Union[PortConnect, None]:
    try:
...
def open_port(host: IPv4Address, port: int, timeout: int=5, backlog: int=1) -> Generator:
    try:
