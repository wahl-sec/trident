...
def setup_logging(args: Namespace, config: Dict[AnyStr, Union[AnyStr, Dict]]) -> None:
    ...
    if config["logging_level"] not in MODIFIED_LOGGER_CONFIG["handlers"]:
        logger.warning(
            f"Unrecognized logging level: '{config['logging_level']}' given by config file, setting to 'INFO'."
        )
        config["logging_level"] = "INFO"

    MODIFIED_LOGGER_CONFIG["loggers"]["__main__"]["handlers"] = [
        config["logging_level"]
    ]
    MODIFIED_LOGGER_CONFIG["loggers"]["__main__"]["level"] = config["logging_level"]

    if args.verbose or (
        "verbose" in config["args"]["trident"] and config["args"]["trident"]["verbose"]
    ):
        MODIFIED_LOGGER_CONFIG["loggers"]["__main__"]["handlers"] = ["DEBUG"]
        MODIFIED_LOGGER_CONFIG["loggers"]["__main__"]["level"] = "DEBUG"

    if args.quiet or (
        "quiet" in config["args"]["trident"] and config["args"]["trident"]["quiet"]
    ):
    ...
    config = setup_trident_arguments(
        {
            "trident": {
                ...
                if k in ["logging_level", "verbose", "quiet"] and v is not None
            }
        },
        config,
    )
    ...
    config = setup_plugin_arguments(
        {
            "store": {
                k: v
                for k, v in vars(args).items()
                if k in ["no_store", "global_store", "path_store"] and v is not None
            },
            "runner": {
                k: v
                for k, v in vars(args).items()
    ...
    config = setup_daemon_arguments(
        {
            "daemon": {
                k: v
                for k, v in vars(args).items()
                if k in ["workers"] and v is not None
            }
        },
        config,
    )
...
if __name__ == "__main__":
    ...
    config = apply_runtime_arguments(
    ...
    logger.info(
        f"Trident finished execution on: '{datetime.now().strftime('%d/%m/%Y %H:%M:%S')}' after: '{round(time.time() - start_time, 5)}' seconds."
    )
