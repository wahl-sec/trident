...
from typing import Union, AnyStr, Iterable, NewType, Tuple, Dict, Any
...
from trident.lib.notification.handler import TridentNotificationHTTPHandlerConfig, TridentNotificationEmailHandlerConfig, TridentNotificationHTTPHandler, TridentNotificationEmailHandler
...
class TridentNotificationDaemonConfig:
    ...
    def __init__(self, runner: TridentRunner, notifications: Dict[AnyStr, Dict[AnyStr, Any]]):
        self.handlers = self._resolve_notification_handlers(notifications, runner.runner_id)

    def _resolve_notification_handlers(self, notifications: Dict[AnyStr, Dict[AnyStr, Any]], runner_id: AnyStr) -> Iterable[TridentNotificationTypeHandler]:
        ...
        for notification_name in notifications:
            for (notification_type, notification_config) in notifications[notification_name].items():
                try:
                    handler_classes = {
                        "http": {"config": TridentNotificationHTTPHandlerConfig, "handler": TridentNotificationHTTPHandler},
                        "email": {"config": TridentNotificationEmailHandlerConfig, "handler": TridentNotificationEmailHandler}
                    }[notification_type.lower()]
                except KeyError:
                    logger.warning(f"Unsupported notification type: '{notification_type}' for notification: '{notification_name}' in runner: '{runner_id}'")
                    continue
                ...
                try:
                    ...
                    logger.error(f"Failed to initialize notification handler: '{notification_name}' for runner: '{runner_id}'")
                    raise e
...
class TridentNotificationDaemon:
    ...
    def send_notification(self, content):
        ...
        for handler in self.daemon_config.handlers:
            try:
                logger.debug(f"Sending notification for handler: '{handler.notification_config.name}'")
                handler.send_notification(content=content if handler.notification_config.include_result else None)
            except Exception as e:
                logger.error(f"Failed to send notification for handler: '{handler.notification_config.name}' due to: {e}")
