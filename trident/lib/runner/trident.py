    ...
    def _initialize_runner_plugin(self) -> Union[Tuple[PluginClass, Module], None]:
        ...
        try:
            ...
            if self.plugin_name not in dir(plugin_module):
                raise ValueError(
                    f"Failed to find a class named: '{self.plugin_name}' in module: '{self.plugin_path}'"
                )
            ...
            logger.error(
                f"Failed to initialize plugin: '{self.plugin_path}' with error: {e}"
            )
            raise e
    ...
    def start_runner(self) -> NoReturn:
        ...
        try:
            ...
            if (
                "thread_event"
                not in signature(
                    self.runner_config.plugin_instance.execute_plugin
                ).parameters
            ):
                logger.warning(
                    f"Thread event parameter not specified in 'execute_plugin' method for plugin: '{self.runner_config.plugin_name}' at '{self.runner_config.plugin_path}'"
                )
                runner_generator = self.runner_config.plugin_instance.execute_plugin(
                    **self.runner_config.plugin_args
                )
            else:
                runner_generator = self.runner_config.plugin_instance.execute_plugin(
                    thread_event=self.runner_config.thread_event,
                    **self.runner_config.plugin_args,
                )
